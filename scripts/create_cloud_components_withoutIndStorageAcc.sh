...
storage_account=$(echo "${rg_name,,}vhdstorageacc"| sed "s/[!@#$%^&*()~\._-'{}\[|;:\"]//g")
...
## create storage account  recommended for unmanaged disks
...
--settings ${CONF_PATH}/azureMountDisk-mgmt-script-config.json \
...
--name customScript \
--publisher Microsoft.Azure.Extensions \
--settings ${CONF_PATH}/azureMountDisk-data-script-config.json \
--protected-settings ${CONF_PATH}/protected-storage-config.json
exit_if_error
